...
class FunctionalCompletableFutureInvokeUtilsTest {
    ...
    class AsyncActionTests {
        ...
        void shouldInvokeAsyncActionWithoutArgs() throws ExecutionException, InterruptedException, TimeoutException {
            Executor executor = Runnable::run;

            AtomicBoolean flag = new AtomicBoolean(false);
            Action action = () -> flag.set(true);
            invokeAsync(action).get(1, TimeUnit.SECONDS);
            assertThat(flag.get()).isTrue();

            flag.set(false);
            invokeAsync(action, executor).get(1, TimeUnit.SECONDS);
            assertThat(flag.get()).isTrue();
        }

        @Test
        @DisplayName("应异步执行单参数 Action")
        void shouldInvokeAsyncAction1Arg() throws ExecutionException, InterruptedException, TimeoutException {
            Executor executor = Runnable::run;

            AtomicInteger result = new AtomicInteger();
            Action1Arg<Integer> action1 = result::set;
            invokeAsync(action1, 10).get(1, TimeUnit.SECONDS);
            assertThat(result.get()).isEqualTo(10);

            invokeAsync(action1, 20, executor).get(1, TimeUnit.SECONDS);
            assertThat(result.get()).isEqualTo(20);
        }

        @Test
        @DisplayName("应异步执行双参数 Action")
        void shouldInvokeAsyncAction2Args() throws ExecutionException, InterruptedException, TimeoutException {
            Executor executor = Runnable::run;

            AtomicInteger result = new AtomicInteger();
            Action2Args<Integer, Integer> action2 = (a, b) -> result.set(a + b);
            invokeAsync(action2, 3, 4).get(1, TimeUnit.SECONDS);
            assertThat(result.get()).isEqualTo(7);

            invokeAsync(action2, 5, 6, executor).get(1, TimeUnit.SECONDS);
            assertThat(result.get()).isEqualTo(11);
        }

        @Test
        @DisplayName("应异步执行三参数 Action")
        void shouldInvokeAsyncAction3Args() throws ExecutionException, InterruptedException, TimeoutException {
            Executor executor = Runnable::run;

            AtomicInteger result = new AtomicInteger();
            Action3Args<Integer, Integer, Integer> action3 = (a, b, c) -> result.set(a + b + c);
            invokeAsync(action3, 1, 2, 3).get(1, TimeUnit.SECONDS);
            assertThat(result.get()).isEqualTo(6);

            invokeAsync(action3, 2, 3, 4, executor).get(1, TimeUnit.SECONDS);
            assertThat(result.get()).isEqualTo(9);
        }

        @Test
        @DisplayName("应异步执行四参数 Action")
        void shouldInvokeAsyncAction4Args() throws ExecutionException, InterruptedException, TimeoutException {
            Executor executor = Runnable::run;

            AtomicInteger result = new AtomicInteger();
            Action4Args<Integer, Integer, Integer, Integer> action4 = (a, b, c, d) -> result.set(a + b + c + d);
            invokeAsync(action4, 1, 2, 3, 4).get(1, TimeUnit.SECONDS);
            assertThat(result.get()).isEqualTo(10);

            invokeAsync(action4, 2, 3, 4, 5, executor).get(1, TimeUnit.SECONDS);
            assertThat(result.get()).isEqualTo(14);
        }

        @Test
        @DisplayName("应异步执行五参数 Action")
        void shouldInvokeAsyncAction5Args() throws ExecutionException, InterruptedException, TimeoutException {
            Executor executor = Runnable::run;

            AtomicInteger result = new AtomicInteger();
            Action5Args<Integer, Integer, Integer, Integer, Integer> action5 = (a, b, c, d, e) -> result.set(a + b + c + d + e);
            invokeAsync(action5, 1, 2, 3, 4, 5).get(1, TimeUnit.SECONDS);
            assertThat(result.get()).isEqualTo(15);

            invokeAsync(action5, 2, 3, 4, 5, 6, executor).get(1, TimeUnit.SECONDS);
            assertThat(result.get()).isEqualTo(20);
        }

        @Test
        @DisplayName("应异步执行六参数 Action")
        void shouldInvokeAsyncAction6Args() throws ExecutionException, InterruptedException, TimeoutException {
            Executor executor = Runnable::run;

            AtomicInteger result = new AtomicInteger();
            Action6Args<Integer, Integer, Integer, Integer, Integer, Integer> action6 = (a, b, c, d, e, f) -> result.set(a + b + c + d + e + f);
            invokeAsync(action6, 1, 2, 3, 4, 5, 6).get(1, TimeUnit.SECONDS);
            assertThat(result.get()).isEqualTo(21);

            invokeAsync(action6, 2, 3, 4, 5, 6, 7, executor).get(1, TimeUnit.SECONDS);
            assertThat(result.get()).isEqualTo(27);
        }

        @Test
        @DisplayName("应异步执行七参数 Action")
        void shouldInvokeAsyncAction7Args() throws ExecutionException, InterruptedException, TimeoutException {
            Executor executor = Runnable::run;

            AtomicInteger result = new AtomicInteger();
            Action7Args<Integer, Integer, Integer, Integer, Integer, Integer, Integer> action7 =
                    (a, b, c, d, e, f, g) -> result.set(a + b + c + d + e + f + g);
            invokeAsync(action7, 1, 2, 3, 4, 5, 6, 7).get(1, TimeUnit.SECONDS);
            assertThat(result.get()).isEqualTo(28);

            invokeAsync(action7, 2, 3, 4, 5, 6, 7, 8, executor).get(1, TimeUnit.SECONDS);
            assertThat(result.get()).isEqualTo(35);
        }

        @Test
        @DisplayName("应异步执行八参数 Action")
        void shouldInvokeAsyncAction8Args() throws ExecutionException, InterruptedException, TimeoutException {
            Executor executor = Runnable::run;
            ...
            invokeAsync(action8, 1, 2, 3, 4, 5, 6, 7, 8).get(1, TimeUnit.SECONDS);
            assertThat(result.get()).isEqualTo(36);

            invokeAsync(action8, 2, 3, 4, 5, 6, 7, 8, 9, executor).get(1, TimeUnit.SECONDS);
